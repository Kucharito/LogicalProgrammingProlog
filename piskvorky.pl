...
tp:- p(S1,' '),
    ...
    retract(p(S1,' ')),
    assert(p(S1,'x')),
    write(([S1,'petica xxxxx']),nl),
    vyhra(x,_),
    vypis_pole.

tp:- 
    p(S,' '),                         % najde nejake prazdne policko
    retract(p(S,' ')),                % odstrani fakt, ze je prazdne
    assert(p(S,'x')),                 % zapise na toto policko znak x
    vypis_pole,                       % vypise aktualny stav pola
    nl,
    write(S).                         % vypise suradnicu vykonaneho tahu
...
o2(2,[X,Y],[X1,Y1],[X2,Y2],[X3,Y3],[X4,Y4]):- X1 is X+1, Y1 is Y+1, X2 is X+2, Y2 is Y+2, X3 is X+3, Y3 is Y+3, X4 is X+4, Y4 is Y+4. % 5 na diagonále HORE
o3(3,[X,Y],[X,Y1],[X,Y2],[X,Y3],[X,Y4]):- Y1 is Y+1, Y2 is Y+2, Y3 is Y+3, Y4 is Y+4, Y5 is Y+5. % | STLPEC SMER HORE
o4(4,[X,Y],[X1,Y1],[X2,Y2],[X3,Y3],[X4,Y4]):- X1 is X+1, Y1 is Y-1, X2 is X+2, Y2 is Y-2, X3 is X+3, Y3 is Y-3, X4 is X+4, Y4 is Y-4. % 5 na diagonále dole
